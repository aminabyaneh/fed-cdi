...
class Experiment(ExperimentBase):
    ...
    def interval(self):
        ...
        """Accessible Dataset Segment"""
        settings_dir = os.path.join('work', self.a.store_folder, f'experiment_{self.a.experiment_id}_settings')
        os.makedirs(settings_dir, exist_ok=True)

        client_settings_dir = os.path.join(settings_dir, f'settings_{self.a.client_id}.npy')
        if os.path.exists(client_settings_dir):
            settings = np.load(client_settings_dir, allow_pickle=True)
            logging.info(f'Detected a stored setting for client {self.a.client_id}.')
            accessible_indices_observational = settings[1]
            accessible_indices_interventional = settings[2]
        else:
            logging.info(f'No settings detected in {client_settings_dir}, Generating one...')

            acc_p = self.a.accessible_percentage / 100

            accessible_indices_observational = [index + self.a.client_id for index in range(self.a.train_functional)
                                                if index % self.a.num_clients == 0]
            accessible_indices_observational = \
                accessible_indices_observational[0: int(acc_p * len(accessible_indices_observational))]

            accessible_indices_interventional = [index + self.a.client_id for index in range(self.a.xfer_epi_size)
                                                 if index % self.a.num_clients == 0]
            accessible_indices_interventional = \
                accessible_indices_interventional[0: int(acc_p * len(accessible_indices_interventional))]

            np.save(client_settings_dir, (self.a.accessible_percentage, accessible_indices_observational,
                                          accessible_indices_interventional, self.a.client_id))

            logging.info(f'Client {self.a.client_id}: Settings generated.\n\n')

        logging.info(f'Client {self.a.client_id}: Starting Epoch {self.S.epochNum}')
        ...
        for q in self.brk(range(self.a.dpe)):
            ...
            if self.a.train_functional:
                ...
                for b, (batch, config) in self.brk(enumerate(zip(smpiter, cfgiter)), max=self.a.train_functional):
                    ...
                    """Access Certain Segment of Dataset"""
                    if b in accessible_indices_observational:
            ...
            for j in self.brk(range(self.a.ipd)):
                ...
                with self.intervention() as intervention:
                    ...
                    with torch.no_grad():
                        ...
                        for b, batch in self.brk(enumerate(smpiter), max=self.a.xfer_epi_size):
                            """Access Certain Segment of Dataset"""
                            if b in accessible_indices_interventional:
                                gammagrad = 0
                                logregret = 0
        ...
        """Exit"""
        logging.info(f'Client {self.a.client_id}: Epoch {self.S.epochNum} done\n')

        # Save gamma
        if self.S.epochNum == self.a.num_epochs - 1:
            # Create a folder if not already there
            save_dir = os.path.join('work', self.a.store_folder, f'experiment_{self.a.experiment_id}')
            os.makedirs(save_dir, exist_ok=True)

            # Save numpy arrays
            np.save(os.path.join(save_dir, f'Gamma_Data_{self.a.round_id}_{self.a.client_id}'),
                    (self.a.accessible_percentage, siggamma.detach().numpy(), self.a.client_id))
            logging.info(f'Client {self.a.client_id}: Experiment data saved\n\n')
